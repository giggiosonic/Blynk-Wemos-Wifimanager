...
server->on(String(F("/firmware")), std::bind(&WiFiManager::handleFirmware, this));
...
void WiFiManager::handleRoot() {
  ...
  page += String(F("<h1>"));
  page += F("Smart IoT device");
  page += String(F("</h1>"));
  page += String(F("<h2 align=\"center\">"));
  page += _apName;
  page += String(F("</h2>"));
  page += String(F("<h3>Configurar red WI-FI y TOKEN BLYNK</h3>"));
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  if (scan) {
    ...
    if (n == 0) {
      ...
      page += F("No se encontraron redes WI-FI. Actualiza la pagina para volver a escanear.");
    } else {
...
void WiFiManager::EC_read(){
   ...
   for( int i=0; i<sz1; i++ ){
    ...
    uint16_t src = EC_SRC();
    if(EC_Config.STRC == src ){
      DEBUG_WM(F("EEPROM Config is correct")); 
      DEBUG_WM(EC_Config.STRC);   
           }
    else {
      DEBUG_WM(F("EEPROM SRC is not valid"));   
        EC_save();
    }   
}

const char* WiFiManager::getAuth() {
  return EC_Config.authtok;
}


void WiFiManager::EC_save(){
   EC_Config.STRC = EC_SRC();
   size_t sz1 = sizeof(EC_Config);
   for( int i=0; i<sz1; i++)
      EEPROM.write(i, *((uint8_t*)&EC_Config+ i));
   EEPROM.commit(); 
   DEBUG_WM(F("Save Config to EEPROM. SCR=%d\n"));    
   DEBUG_WM(EC_Config.STRC);
}


/** Handle the WLAN save form and redirect to WLAN config page again */
void WiFiManager::handleTokenSave() {
  DEBUG_WM(F("Token save"));

//String value = server->arg("t").c_str();

strcpy(EC_Config.authtok, server->arg("t").c_str());

    
//   value.toCharArray(authtok, 48);
    DEBUG_WM(F("Parameter"));
...
String page = FPSTR(HTTP_HEADER);
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += FPSTR(HTTP_SAVED_TOK);
  page += "<br/><form action=\"/\" method=\"post\"><button>Atras</button></form>";
    page += FPSTR(HTTP_END);
  server->sendHeader("Content-Length", String(page.length()));
  server->send(200, "text/html", page);
  EC_save();
  DEBUG_WM(F("Sent token save page"));
  DEBUG_WM(EC_Config.authtok);
}

uint16_t WiFiManager::EC_SRC(){
   uint16_t src = 0;
   size_t sz1 = sizeof(EC_Config);
   uint16_t src_save = EC_Config.STRC;
   EC_Config.STRC = 0;
   for( int i=0; i<sz1; i++)src +=*((uint8_t*)&EC_Config+ i);
   EC_Config.STRC = src_save;
   return src;  
   //DEBUG_WM(src);
}


/** Handle the TOKEN page */
void WiFiManager::handleToken() {
  DEBUG_WM(F("TOKEN"));

  String page = FPSTR(HTTP_HEADER);
  page.replace("{v}", "Token BLYNK");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += FPSTR(HTTP_FORM_TOK);
  char parLength[5];
  page += FPSTR(HTTP_FORM_END);
  page += "<br/><form action=\"/\" method=\"post\"><button>Atras</button></form>";
  page += FPSTR(HTTP_END);
...
void WiFiManager::handleUpdate() {
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += FPSTR(HTTP_FORM_UPDATE);
  char parLength[7];
  page += FPSTR(HTTP_FORM_FIRMWARE);
  page += "<br/><form action=\"/\" method=\"post\"><button>Volver</button></form>";
  page += FPSTR(HTTP_END);
   
  server->sendHeader("Content-Length", String(page.length()));
  server->send(200, "text/html", page);
  
  DEBUG_WM(F("Sent update page"));
}

/** Handle the Firmware page */
void WiFiManager::handleFirmware() {
  DEBUG_WM(F("Firmware"));

  String page = FPSTR(HTTP_HEADER);
  page.replace("{v}", "firmware RUN");
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += F("Module will reset and UPDATE in a few seconds.");
  page += FPSTR(HTTP_END);

  server->sendHeader("Content-Length", String(page.length()));
  server->send(200, "text/html", page);
  DEBUG_WM(F("Sent firmware page"));
...
void WiFiManager::handleReset() {
  ...
  page += _customHeadElement;
  page += FPSTR(HTTP_HEADER_END);
  page += F("El dispositivo ahora se reiniciara.");
  page += FPSTR(HTTP_END);
